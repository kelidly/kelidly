...
<!--          <li><a href="#tab1" class="default-tab">管理列表</a></li>
        ...
        <a href="../news/news_pub.jsp">news_pub</a></ul>
        <div class="clear"></div>
      </div>
	...
	function submitform(obj){
		...
		var url = "<%=basePath%>/back/product/delMany";
...
<form action="<%=basePath%>/back/product/pageQuery" id="pageForm_search" method="get">
    ...
    <!--               <div class="pagination"> 
            ...
            <form action=""  method="post" name="checkbox_Form" id="checkbox_Form" >
              <c:forEach var="product"  varStatus="status" items="${pagemodel.result}"  >
               <tr>
                <td>
                    <input type="checkbox" name="checkId" id="checkId" value="${product.id}"/>
	                     
                </td>
                <td>${status.index +1 } </td>
                <td>${product.typeBean.type}</td>
                <td>${product.name }</td>
                <td>${product.model }</td>
                <td>${product.picture}</td>
                <td>
                  <!-- Icons -->
                  <a href="<%=basePath%>/back/product/updateView?pid=${product.id}" title="编辑"><img src="<%=basePath %>/common/admin/images/icons/pencil.png" alt="Edit" /></a> <a href="<%=basePath%>/back/product/del?pid=${product.id}" title="删除"><img src="<%=basePath %>/common/admin/images/icons/cross.png" alt="Delete" /></a> <a href="#" title="设置"><img src="<%=basePath %>/common/admin/images/icons/hammer_screwdriver.png" alt="Edit Meta" /></a> </td>
              </tr>
              </c:forEach>
             </form>     
