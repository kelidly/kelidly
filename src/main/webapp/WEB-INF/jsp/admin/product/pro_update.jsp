 ...
 <!--         <li><a href="<%=basePath %>/back/product/manage" >管理列表</a></li>
      ...
      <div class="content-box-content">
        ...
        <div class="tab-content default-tab" id="tab1">
          <form action="<%=basePath%>/back/product/update" method="post" enctype="multipart/form-data">
            <input type="hidden" name="id" value="${bean.id }">
            <fieldset>
            <p>
              <label>产品名称</label>
              <input class="text-input medium-input datepicker" value="${bean.name }" type="text" id="medium-input" name="name" />
  ...
  <c:forEach var="typeBean" items="${typeList}">
               ...
               <option  <c:if test="${typeBean.id == bean.typeBean.id}"> selected  </c:if>
  ...
  <c:forEach var="usesetBean" items="${bean.productUseSet}">
              <c:if test="${usesetBean.id==useBean.id}" > checked="checked" </c:if>
              </c:forEach>
              />
             ${useBean.name }<br />
            ...
            <p>
              <label>产品型号</label>
              <input value="${bean.model }" class="text-input medium-input datepicker" type="text"  name="model" />
              <!--<span class="input-notification success png_bg">成功通过提示信息</span>
              <span class="input-notification error png_bg">错误提示信息</span>
              -->
              <br />
              ...
              <br />
              <img   src="<%=basePath %>/${bean.picture}"/>
   
              </p>
            <p>
              <label>简介</label>
              <textarea class="text-input textarea wysiwyg" id="textarea" name="intro" cols="79" rows="12">
              ${bean.intro}
              </textarea>
            </p>
