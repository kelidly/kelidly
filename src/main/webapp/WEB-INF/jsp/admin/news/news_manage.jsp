...
<!--          <li><a href="#tab1" class="default-tab">管理列表</a></li>
	...
	function submitform(obj){
		...
		var url = "<%=basePath%>/back/news/delMany";
...
<form action="<%=basePath%>/back/news/pageQuery" id="pageForm_search" method="get">
    ...
    <!--               <div class="pagination"> 
            ...
            <form action=""  method="post" name="checkbox_Form" id="checkbox_Form"  >
              <c:forEach var="news"  varStatus="status" items="${pagemodel.result}"  >
               <tr>
                <td>
                    <input type="checkbox" name="checkId" id="checkId" value="${news.id}"/>
	                     
                </td>
                <td>${status.index +1 } </td>
                <td>${news.newsType.type}</td>
                <td>${news.title }</td>
                <td>${news.pubTime }</td>
                <td>${news.hot}</td>
                <td>
                  <!-- Icons -->
                  <a href="<%=basePath%>/back/news/updateView?newsId=${news.id}" title="编辑"><img src="<%=basePath %>/common/admin/images/icons/pencil.png" alt="Edit" /></a> <a href="<%=basePath%>/back/news/del?newsId=${news.id}" title="删除"><img src="<%=basePath %>/common/admin/images/icons/cross.png" alt="Delete" /></a> <a href="#" title="设置"><img src="<%=basePath %>/common/admin/images/icons/hammer_screwdriver.png" alt="Edit Meta" /></a> </td>
              </tr>
              </c:forEach>
             </form>     
