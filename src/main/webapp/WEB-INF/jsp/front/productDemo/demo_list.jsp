...
<li><A href='<%=basePath %>/productDemo/typeShow?typeId=${bean.id}&useId=${bean.useBean.id}'>${bean.type}</A></li> 
</c:forEach>
</ul>
</div>
</div>
...
<br>
<div class="content">
      <c:forEach items="${demoMap}" var="entry" >

<h3>${entry.key.type}</h3>
            <div class="MorePro">
              <div class="CaseBlock">
                <ul>
                  <ul>    
                  <c:forEach items="${entry.value}" var="bean" >
                    <div class='albumblock'>
                      <div class='inner'><a href='<%=basePath%>/productDemo/detail?id=${bean.id}&typeId=${bean.typeBean.id}&useId=${bean.typeBean.useBean.id}' target='_blank'  title='${bean.name}'><img src='<%=basePath%>/${bean.pic}' />
                        <div class='albumtitle'>${bean.name}</div>
                        </a></div>
                    </div>  
                    </c:forEach>             
                  </ul>
