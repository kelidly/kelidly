...
(function ($) {
    ...
    function createImg(key, options, file) {
        ...
        if (img.size() == 0) {
            ...
            img = img.find('[tag="reviewContent"]').children().eq(0);
        }
        return img;
    }
    function S4() {
    ...
    function hasTemplate(options) {
        return options.template != null && options.template.length > 0;
    }
    function hasTarget(options) {
        return options.target == null || options.target.length < 2;
    }
})(jQuery);
